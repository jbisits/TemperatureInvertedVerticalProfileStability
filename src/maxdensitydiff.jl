...
function series_max_Δρ(raster_series::RasterSeries, ΔΘ_thres::Float64; zdepth = -1000.0)
    ...
    for (i, stack) ∈ enumerate(raster_series)
        ...
        converted_rs = convert_ocean_vars(stack, (Sₚ = :SALT, θ = :THETA))

        profile_max_res = profiles_Δρ_max(converted_rs, zdepth, ΔΘ_thres)
        var_mats[1] = profile_max_res.Δρ_cab_max
        var_mats[2] = profile_max_res.Δρ_static_max
        var_mats[3] = Θ_upper(converted_rs[:Θ], profile_max_res.upper_level_idx)
        var_mats[4] = Θ_lower(converted_rs[:Θ], profile_max_res.lower_level_idx)
        var_mats[5] = ΔΘ(converted_rs[:Θ], profile_max_res.upper_level_idx,
